...
export const RationalizationTool: React.FC = () => {
  ...
  const handleRationalize = () => {
    try {
      ...
      // Simple placeholder logic for rationalization
      // This shows the process without complex computation
      const denom = denominator.trim();

      if (!denom.includes('sqrt')) {
        setError('El denominador debe contener una raíz (sqrt). Ej: sqrt(2), sqrt(3)');
        return;
      }

      const steps_temp: string[] = [
        `\\text{Expresión original: } \\frac{${numerator}}{${denom}}`,
        `\\text{Multiplicar por } \\frac{${denom}}{${denom}}`,
      ];

      setSteps(steps_temp);
      setResult(`\\frac{${numerator} \\cdot ${denom}}{(${denom})^2}`);
    } catch (err: any) {
      setError(err.message || 'Error al racionalizar');
      setResult('');
      setSteps([]);
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
        <div className="rounded-lg border px-4 py-3 border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white">
          Escribe raíces como: sqrt(2), sqrt(3)
        </div>
        <div className="rounded-lg border px-4 py-3 border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white">
          Ejemplo: <MathText expression="\\frac{1}{\\sqrt{2}}" className="inline-block" />
        </div>
      </div>
